...
class MemberDef : virtual public Definition
{
 ...
 typedef struct auto_generate_s
 {
  QCString name;    // member name 
  QCString arg;    
  QCString member;  // kind of member : function, variable, macro, struct 
  QCString t;
  QCString d_name;  // file name 
  QCString type;    // return type  
  QCString absFile; // absolute file name 
  int no_line;      // item position number
  bool show;
  struct auto_generate_s* next;
...
static int no;
  ...
  public:
    ...
    //
    // ---- actions about undocumented item ------
    //
    
    virtual void autoGenerateComment( _auto_generate_s &lags) const = 0;
    virtual void saveUndocummentedItem( const ClassDef     *cd, 
