...
void MemberDefImpl::warnIfUndocumented() const 
{
  ...
  if (!hasUserDocumentation() && extractAll==true)
  {
...
void MemberDefImpl::saveUndocummentedItem( const ClassDef     *cd, 
  					   const NamespaceDef *nd, 
  					   const FileDef      *fd,
					   const GroupDef     *gd,
					   const Definition   *d,_auto_generate_s& lags) const
{ 
  ...
  outputDir=outputDir+"/undocumented.txt";
  ofstream output;
  stringstream ss;
  ss << no <<" ("<<plags->prev->no_line<<") "<<plags->prev->absFile.data()<<" "<<plags->prev->type.data()<<" "<<plags->prev->name.data()<<" "<<plags->prev->arg.data();
